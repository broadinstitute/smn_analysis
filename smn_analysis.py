...
for cram_path in cram_paths:
    ...
    records.update({"SAMPLEID": cram_filename})
    #looping to read exon coordinates from the list
    for exon_region in exon_coordinates:
        SMN1_exon_read_count = cram.count(region= exon_region[0])
        SMN2_exon_read_count = cram.count(region= exon_region[1])
        exon = "SMN1 + SMN2 exon " + str(exon_coordinates.index(exon_region) + 1) + " reads"
        exon_read = {exon : SMN1_exon_read_count + SMN2_exon_read_count}
        records.update(exon_read)
